...
CREATE TABLE `organizations` (
	...
	CONSTRAINT `organizations_organizationId_userId_pk` PRIMARY KEY(`organizationId`,`userId`),
	CONSTRAINT `profile_is_organization` CHECK(`organizations`.`organizationId` = 'organization')
);
--> statement-breakpoint
CREATE TABLE `post` (
	`id` varchar(255) NOT NULL,
...
ALTER TABLE `organizations` ADD CONSTRAINT `organizations_organizationId_profile_id_fk` FOREIGN KEY (`organizationId`) REFERENCES `profile`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `organizations` ADD CONSTRAINT `organizations_userId_user_id_fk` FOREIGN KEY (`userId`) REFERENCES `user`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
